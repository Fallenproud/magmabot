---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for MagmaBot deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, MagmaBot cron scheduling for periodic checks, or version status checks on a machine running MagmaBot (laptop, workstation, Pi, VPS).
---

# MagmaBot Host Hardening

## Overview

Assess and harden the host running MagmaBot, then align it to a user-defined risk tolerance without breaking access. Use MagmaBot security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim MagmaBot changes the host firewall, SSH, or OS updates; it does not.
...
5. MagmaBot gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if MagmaBot can access sensitive data.
...
### 2) Run MagmaBot security audits (read-only)

As part of the default read-only checks, run `MagmaBot security audit --deep`. Only offer alternatives if the user requests them:

1. `MagmaBot security audit` (faster, non-probing)
2. `MagmaBot security audit --json` (structured output)

Offer to apply MagmaBot safe defaults (numbered):

1. `MagmaBot security audit --fix`

Be explicit that `--fix` only tightens MagmaBot defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check MagmaBot version/update status (read-only)

As part of the default read-only checks, run `MagmaBot update status`.
...
- Credential hygiene notes (location of MagmaBot creds, prefer disk encryption)
...
- MagmaBot security audit (re-run)
...
After MagmaBot install or first hardening pass, run at least one baseline audit and version check:

- `MagmaBot security audit`
- `MagmaBot security audit --deep`
- `MagmaBot update status`

Ongoing monitoring is recommended. Use the MagmaBot cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `MagmaBot cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `MagmaBot update status`
...
Before creating, `MagmaBot cron list` and match on exact `name`. If found, `MagmaBot cron edit <id> ...`.
If not found, `MagmaBot cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `MagmaBot update status` (preferred for source checkouts and channels)
2. `npm view MagmaBot version` (published npm version)

## MagmaBot command accuracy

Use only supported commands and flags:

- `MagmaBot security audit [--deep] [--fix] [--json]`
- `MagmaBot status` / `MagmaBot status --deep`
- `MagmaBot health --json`
- `MagmaBot update status`
- `MagmaBot cron add|list|runs|run`

Do not invent CLI flags or imply MagmaBot enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by MagmaBot compaction:
